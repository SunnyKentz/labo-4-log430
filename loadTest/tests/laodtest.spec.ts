...
test('load testing', async ({ page }) => {
    ...
    let i = 0;
    while (i < 500) {
        await page.getByRole('textbox', { name: 'Rechercher un produit...' }).fill('d');
...
test('load testing 3', async ({ page }) => {
    ...
    let i = 0;
    while (i < 500) {
        await page.getByRole('textbox', { name: 'Rechercher un produit...' }).clear();
        await page.getByRole('textbox', { name: 'Rechercher un produit...' }).fill('s');
        await page.getByRole('textbox', { name: 'Rechercher un produit...' }).press('Enter');
        await page.locator('.mdf-btn').first().click();
        await page.locator('input[name="nom"]').fill('s');
